...
class Qwen2VLGRPOVLLMTrainerModified(Trainer):
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id:
                ...
                model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                    model, local_files_only=True, **model_init_kwargs
                )
                ...
                '''
                RuntimeError: Error(s) in loading state_dict for Qwen2VLForConditionalGeneration:
                size mismatch for visual.merger.mlp.2.weight: copying a param with shape torch.Size([2048, 5120]) from checkpoint, the shape in current model is torch.Size([1280, 5120]).
                size mismatch for visual.merger.mlp.2.bias: copying a param with shape torch.Size([2048]) from checkpoint, the shape in current model is torch.Size([1280]).
                You may consider adding `ignore_mismatched_sizes=True` in the model `from_pretrained` method.
                '''
                model = Qwen2VLForConditionalGeneration.from_pretrained(model, **model_init_kwargs, ignore_mismatched_sizes=True) ## TODO 忽略不对等的参数会导致问题
        else:
        ...
        if peft_config is not None:
            print('WORKING WITH PEFT MODEL')
            model = get_peft_model(model, peft_config)

        # Reference model
        if is_deepspeed_zero3_enabled():
            print('ENABLE DEEPSPEED ZERO3')
            if "Qwen2-VL" in model_id:
        ...
        if processing_class is None:
            if "Qwen" in model_id or "Aria" in model_id:
                print('WORKING WITH PROCESSING CLASS')
    ...
    def _get_per_token_logps(
        ...
        pixel_values = pixel_values.to(model.device) # cuda:0
        image_grid_thw = image_grid_thw.to(device=model.device)
        # pixel_values = pixel_values.to("cuda:0") # cuda:0
        # image_grid_thw = image_grid_thw.to(device="cuda:0")
        logits = model(
            input_ids,
